<template>
  <div>
      ...
      <div ref="chartSec" style="width: 100%; height: 400px;"></div>
  </div>
</template>
...
export default {
  ...
  data() {
      return {
          ...
          dateFirstRange: [],  // Stores dates for the x-axis
          firstSeriesData: [], // Stores data for the series
          flag: false,         // Flag to indicate first processing
          colorArray: ['#C5D6B6', '#81C2A7', '#4CB2AC', '#326284', '#162C3B'],  // Array of colors for series
          rangeArray: [        // Defines ranges for the data
              { 'min': 0, 'max': 99 },
              { 'min': 100, 'max': 499 },
              { 'min': 500, 'max': 999 },
              { 'min': 1000, 'max': 4999 },
  ...
  methods: {
      ...
      // Processes the raw data into a format suitable for ECharts
      processData(dataObj) {
          this.flag = false;
          let range = null;
      ...
      // Initializes and configures the ECharts instance
      setChart() {
          const echarts = require('echarts');  // Import ECharts
          if (this.chartInstance) {
              this.chartInstance.dispose();  // Dispose of existing chart instance if present
          }
          this.chartInstance = echarts.init(this.$refs.chartSec);  // Initialize new ECharts instance
